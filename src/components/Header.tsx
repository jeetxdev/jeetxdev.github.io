...
const NAV_LINK_CLASS =
  "inline-flex shrink-0 items-center whitespace-nowrap rounded-[10px] px-[14px] py-[9px] text-text-muted transition-colors duration-200 hover:bg-surface-hover hover:text-ink max-[721px]:min-h-11 max-[721px]:px-[8px] max-[721px]:py-[8px] max-[721px]:text-[13px] max-[359px]:px-[4px] max-[359px]:text-[12px]";

export function Header() {
  return (
    <header
      className={cn(
        "glass-header relative sticky top-[14px] z-10 mt-[18px] flex flex-wrap items-center justify-between gap-[18px] rounded-2xl py-3 pr-3 pl-[18px]",
        "rail:static",
    ...
    >
      <div className="flex items-center gap-[11px] font-display text-[15px] font-medium tracking-[-0.01em] max-[721px]:pl-1">
        <span
          aria-hidden="true"
          className="h-2 w-2 rounded-full bg-success shadow-[0_0_12px_var(--color-success)]"
        />
        Jeet Mukherjee
        {/* Below 340px the brand line would run into the theme menu parked
            at the header's top right, so the qualifier drops. */}
        <span className="font-normal text-text-dimmer max-[340px]:hidden">
          / full-stack
        </span>
      </div>
